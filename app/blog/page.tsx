...
//   return (
//     <div className="relative mx-auto mt-24 max-lg:max-w-2xl h-screen">
//       <div className="absolute -top-24 -right-32 isolate z-0 not-xl:hidden">
//         <Keyboard />
//       </div>
//       <GridContainer>
//         <h1 className="mx-2 text-6xl tracking-tighter text-balance sm:text-8xl lg:text-9xl">The Blog</h1>
//       </GridContainer>

//       <GridContainer className="mt-10 font-tomato-sans">
//         <p className="mx-2 text-lg">My thoughts...</p>
//       </GridContainer>
...
//                   <div className="prose prose-blog mt-4 line-clamp-3 leading-7">{"There are a lot of UI blocks in Tailwind Plus that need JavaScript to really be useful, like dialogs, dropdowns, command palettes, and more. And unless you're a React or Vue user, using those UI blocks has always meant writing all of that tricky JavaScript yourself. Well today that finally change"}</div>
//                   <Link
//                     href={`/blog/${slug}`}
...
import GridContainer from "../components/grid-container";
import { Keyboard } from "@/components/keyboard";
import { getPosts, excerptFromBody } from "@/lib/blogApi";
...
export default async function Blog() {
  ...
  return (
    <div className="relative mx-auto mt-24 max-lg:max-w-2xl h-screen">
      ...
      <div className="mt-12 mb-46 grid grid-cols-1 lg:grid-cols-[24rem_2.5rem_minmax(0,1fr)]">
        {posts.length === 0 && (
          <div className="col-span-3 px-2 text-black md:text-gray-500">No posts yet.</div>
        )}
        {posts.map((p, index) => (
          <Fragment key={p.slug}>
            <GridContainer className="col-span-3 grid lg:grid-cols-[24rem_2.5rem_minmax(0,1fr)] divide-x divide-gray-950/5 dark:divide-white/10">
              ...
              {/* Middle gap column */}
              <div />
              {/* Post content (blue) - right column */}
              <div className="px-2 font-tomato-sans">
                <div className="max-w-2xl">
                  <div className="mb-4 font-mono text-sm/6 font-medium tracking-widest text-black md:text-gray-500 uppercase lg:hidden">
                    {p.updatedAt ? formatDate(p.updatedAt) : ""}
                  </div>
                  <Link href={`/blog/${p.slug}`} className="text-xl font-bold">
                    {p.title}
                  </Link>
                  <div className="mt-4 line-clamp-3 leading-7">{excerptFromBody(p.body)}</div>
                  <Link
                    href={`/blog/${p.slug}`}
                    className="mt-4 inline-block text-sm font-semibold text-sky-500 hover:text-sky-600 dark:text-sky-400"
                  >
