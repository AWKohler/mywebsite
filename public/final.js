...
// Define global initialization function that can be called from React
const initD3Visualizations = function() {
  ...
  // Check if all the required DOM elements exist
  if (!document.getElementById('language-chart') || 
      !document.getElementById('time-chart') || 
      !document.getElementById('tech-graph')) {
    console.error('Required DOM elements for D3 visualizations not found!');
    return;
  }

// Original document.addEventListener is wrapped within the function
// so it can be manually called from React
    // =====================================================================
...
if (typeof document !== 'undefined') {
  if (document.readyState === 'complete' || document.readyState === 'interactive') {
    setTimeout(initD3Visualizations, 1);
  } else {
    document.addEventListener('DOMContentLoaded', initD3Visualizations);
  }
}

// Expose to global scope for React to call
if (typeof window !== 'undefined') {
  window.initD3Visualizations = initD3Visualizations;
}